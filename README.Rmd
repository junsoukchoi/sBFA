...
The following example describes how to use the package `sBFA` to do the posterior inference for the sparse Bayesian factor models with the Dirichlet-Laplace priors. 
The function `sBFA_DL` takes data with the number of factor to be fitted and some arguments needed for the Gibbs sampler, and returns MCMC samples from the posterior distribution of the sparse Bayesian factor models with the Dirichlet-Laplace priors. 
